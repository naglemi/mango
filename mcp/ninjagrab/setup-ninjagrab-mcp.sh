...
USABILITY_ROOT="$(dirname "$(dirname "$SCRIPT_DIR")")"
...
if [ ! -f "$USABILITY_ROOT/ninjagrab.sh" ]; then
    echo -e "ninjagrab.sh not found in $USABILITY_ROOT"
...
if claude mcp add ninjagrab -s project -e NINJAGRAB_SCRIPT_PATH="$USABILITY_ROOT/ninjagrab.sh" -- python "$SCRIPT_DIR/ninjagrab_mcp_server.py"; then
